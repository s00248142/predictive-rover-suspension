...
try:
    ...
    while True:
        ...
        #     FL_TRIM_DEG += AUTO_TRIM_GAIN * (pitch_error + roll_error)
        #     FR_TRIM_DEG += AUTO_TRIM_GAIN * (pitch_error - roll_error)
        #     REAR_TRIM_DEG += AUTO_TRIM_GAIN * (-pitch_error)

        #     FL_TRIM_DEG = clamp(FL_TRIM_DEG, -AUTO_TRIM_LIMIT, 
        #                         AUTO_TRIM_LIMIT)
        #     FR_TRIM_DEG = clamp(FR_TRIM_DEG, -AUTO_TRIM_LIMIT, 
        #                         AUTO_TRIM_LIMIT)
        #     REAR_TRIM_DEG = clamp(REAR_TRIM_DEG, -AUTO_TRIM_LIMIT, 
        #                           AUTO_TRIM_LIMIT)
        ...
        if AUTO_TRIM_ENABLED:
            ...
            # slowly return trims toward zero when no longer needed
            FL_TRIM_DEG *= (1.0 - AUTO_TRIM_DECAY)
            FR_TRIM_DEG *= (1.0 - AUTO_TRIM_DECAY)
            REAR_TRIM_DEG *= (1.0 - AUTO_TRIM_DECAY)

            FL_TRIM_DEG = clamp(FL_TRIM_DEG, -20.0, 20.0)
            FR_TRIM_DEG = clamp(FR_TRIM_DEG, -20.0, 20.0)
            REAR_TRIM_DEG = clamp(REAR_TRIM_DEG, -20.0, 20.0)
