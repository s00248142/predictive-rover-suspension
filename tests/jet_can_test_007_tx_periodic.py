...
try:
    ...
    state_tx = bus0.send_periodic(msg, 1.0)
    ...
    while True:
        ...
        #     )
        if rx_msg is not None:
            # print(hex(rx_msg.arbitration_id), list(rx_msg.data))
            # print(hex(rx_msg.arbitration_id), [hex(b) for b in rx_msg.data])
            print(hex(rx_msg.arbitration_id), [f"0x{b:02X}" for b in rx_msg.data])
