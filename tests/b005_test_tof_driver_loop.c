...
int main(void) {
    ...
    status = VL53L4CD_SetRangeTiming(device, 10, 0);
    if  (status != 0) return status;
...
gcc tests/b005_test_tof_driver_loop.c \
tof_driver/platform.c \
tof_driver/VL53L4CD_api.c \
-o tests/b005_test_tof_driver_loop \
-lm
...
./tests/b005_test_tof_driver_loop
