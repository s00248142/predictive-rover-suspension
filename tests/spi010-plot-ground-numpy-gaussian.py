...
try:
    ...
    # x, y = zip(*points)
    # plt.scatter(x, y)
    plt.scatter(points[:,0], points[:,1], s=10, alpha=0.5, label="raw points")
    plt.xlim(-170, 25)
    plt.ylim(-20, 120)

    # Gaussian smoothing
    sigma_samples = 5.0  # smoothing strength in "samples" (try 1-5)
    x_s = gaussian_filter1d(points[:,0], sigma=sigma_samples, mode="nearest")
    y_s = gaussian_filter1d(points[:,1], sigma=sigma_samples, mode="nearest")
    smoothed = np.column_stack([x_s, y_s])
    plt.plot(x_s, y_s, linewidth=2, color='red', label="gaussian smoothed")
