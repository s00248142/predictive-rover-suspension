...
Plug in device and see if it's connected:
```bash
ls /dev/ttyACM*
```
...
Response:
```
user@jetson:~$ ls /dev/ttyACM*
/dev/ttyACM0
```

Check for dialout permissions:
```bash
groups
```
(*Should see 'dialout')<br>
If no ```dialout``` run:
```bash
...
```
To exit press ```ctrl + A```, ```K```, ```Y```.

Clearer serial tool:
```bash
sudo apt install picocom
```
Then run to check details of connection and open a console:
```bash
